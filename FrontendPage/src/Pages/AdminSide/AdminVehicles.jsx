      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-4">
          ...
          <Input
            label="Capacity"
            name="capacity"
            placeholder="e.g. 4 persons / 50 seats"
            value={form.capacity}
            onChange={handleChange}
          />
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <StatCard
            icon="🔴"
            label="In Use"
            value={countByStatus("IN_TRIP") + countByStatus("ON_CALL")}
            accent
          />
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {loading
              ? Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-36 animate-pulse rounded-2xl border border-gray-100 bg-white shadow-card"
                  />
                ))
              : vehicles.map((v) => (
                  <div
                    key={v.id}
                    className="rounded-2xl border border-gray-100 bg-white p-5 shadow-card transition-shadow hover:shadow-card-hover"
                  >
                    <div className="mb-3 flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-cu-red-light text-2xl">
                          {TYPE_ICON[v.vehicleType] || "🚗"}
                        </div>
                        <div>
                          <p className="text-sm font-bold text-gray-800">
                            {v.registrationNo || v.registration_no}
                          </p>
                    ...
                    <div className="mb-4 space-y-1.5 text-xs text-gray-500">
                      <div className="flex gap-2">
                        <span className="w-16 text-gray-300">Driver</span>
                        <span className="font-medium text-gray-700">
                          {v.driverName || v.driver_name || "Unassigned"}
                        </span>
                      </div>
                      <div className="flex gap-2">
                        <span className="w-16 text-gray-300">Capacity</span>
